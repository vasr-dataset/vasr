...
from F_find_CD_pairs import calculate_weighted_srl_score
...
def main():
    ...
    all_train_pairs = set([tuple(x) for x in train_df[['diff_item_A', 'diff_item_B']].values])
    all_testdev_pairs = set([tuple(x) for x in testdev_df[['diff_item_A', 'diff_item_B']].values])
    print(f"# pairs: train: {len(all_train_pairs)}, testdev: {len(all_testdev_pairs)}")

    train_df_ood = train_df[train_df.apply(lambda r: pair_not_in_test(r, all_testdev_pairs), axis=1)]
    print(f"OOD: train: {len(train_df_ood)}")

    if not os.path.exists(testdev_with_extracted_feats_path):
        extract_testdev_jaccard_and_atoms_feats(train_df_ood, testdev_df)
        testdev_df.to_csv(testdev_with_extracted_feats_path, index=False)
        print(f"Extracted feats of {len(testdev_df)} items, dumped to: {testdev_with_extracted_feats_path}\nRun again to create splits")
        exit()
    testdev_df_with_extracted_feats = pd.read_csv(testdev_with_extracted_feats_path)
    test_df, dev_df = create_silver_and_gold_splits(testdev_df_with_extracted_feats, train_df_ood)

    train_df_full_sampled_at_ood = train_df.sample(len(train_df_ood))

    train_df.to_csv(os.path.join(data_path, 'ABCD_matches', f'all_ABCD_matches_rule_based_sampled_train_full.csv'), index=False)
    train_df_ood.to_csv(os.path.join(data_path, 'ABCD_matches', f'all_ABCD_matches_rule_based_sampled_train_ood.csv'), index=False)
    train_df_full_sampled_at_ood.to_csv(os.path.join(data_path, 'ABCD_matches', f'all_ABCD_matches_rule_based_sampled_train_full_at_size_of_ood.csv'), index=False)

    print(f"Dumped train & dev: train_df: {len(train_df)}, train_df_ood: {len(train_df_ood)}, train_df_full_sampled_at_ood: {len(train_df_full_sampled_at_ood)}")
...
def build_dev(NOISE_LEVEL, desired_dev_gold_size_after_annotation, test_df, testdev_df):
    ...
    dev_df_items_to_sample = test_df_percentages.apply(lambda x: int(x * dev_df_ood_all_desired_size) + 1)
    dev_df_items = []
